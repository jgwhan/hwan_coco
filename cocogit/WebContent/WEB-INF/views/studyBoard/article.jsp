...
<style type="text/css">
.studyBoard-article .table {
    margin-top: 15px;
}
.studyBoard-article .table thead tr, .studyBoard-article .table tbody tr {
    height: 30px;
}
.studyBoard-article .table thead tr th, .studyBoard-article .table tbody tr td {
    font-weight: normal;
    border-top: none;
    border-bottom: none;
}
.studyBoard-article .table thead tr {
    border-top: #d5d5d5 solid 1px;
    border-bottom: #dfdfdf solid 1px;
} 
.studyBoard-article .table tbody tr {
    border-bottom: #dfdfdf solid 1px;
}
.studyBoard-article .table i {
    background: #424951;
...
<c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId==dto.userId}">
    ...
    var url = "<%=cp%>/studyBoard/delete.do?" + params;
...
<c:if test="${sessionScope.member.userId==dto.userId}">
    ...
    var page = "${page}";
    var params = "num="+num+"&page="+page;
    var url = "<%=cp%>/studyBoard/update.do?" + params;
...
function listPage(page) {
	var url="<%=cp%>/studyBoard/listReply.do";
	var num="${dto.num}";
...
function sendReply() {
	...
	$.ajax({
		type:"POST"
		,url:"<%=cp%>/studyBoard/insertReply.do"
		,data:params
		,dataType:"json"
...
function deleteReply(replyNum, pageNo, userId) {
	...
	if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
		var url="<%=cp%>/studyBoard/deleteReply.do";
...
<div class="container" role="main">
    <div class="bodyFrame col-sm-11"  style="float:none; margin-left: auto; margin-right: auto;">
	    ...
	    <div class="table-responsive" style="clear: both;">
	                     ...
	                     <tr>
	                         <td colspan="2">
							      ...
							      <c:if test="${not empty preReadDto }">
								      <a href="<%=cp%>/studyBoard/article.do?${params}&num=${preReadDto.num}">${preReadDto.subject}</a>
							       </c:if>
	                         </td>
	                     </tr>
	                     <tr>
	                         <td colspan="2" style="border-bottom: #d5d5d5 solid 1px;">
							      ...
							      <c:if test="${not empty nextReadDto }">
								      <a href="<%=cp%>/studyBoard/article.do?${params}&num=${nextReadDto.num}">${nextReadDto.subject}</a>
							       </c:if>
	                         </td>
	                     </tr>
	                </tbody>
	                	...
	                	<tr>
	                		...
	                		<td align="right">
	                		    <button type="button" class="btn btn-default btn-sm wbtn"
	                		                onclick="javascript:location.href='<%=cp%>/studyBoard/list.do?${params}';"> 목록으로 </button>
	                		</td>
	                	</tr>
	                </tfoot>
	            </table>
	       ...
	       <div class="studyBoard-reply">
	           <div class="studyBoard-reply-write">
	               ...
	               <div style="text-align: right; padding-top: 10px;">
	                   <button type="button" class="btn btn-default btn-sm" onclick="sendReply();"> 댓글등록 <span class="glyphicon glyphicon-ok"></span></button>
	               </div>           
	           </div>
