...
<style type="text/css">
.webboard-article .table {
    margin-top: 15px;
}
.webboard-article .table thead tr, .webboard-article .table tbody tr {
    height: 30px;
}
.webboard-article .table thead tr th, .webboard-article .table tbody tr td {
    font-weight: normal;
    border-top: none;
    border-bottom: none;
}
.webboard-article .table thead tr {
    border-top: #d5d5d5 solid 1px;
    border-bottom: #dfdfdf solid 1px;
} 
.webboard-article .table tbody tr {
    border-bottom: #dfdfdf solid 1px;
}
.webboard-article .table i {
    background: #424951;
...
<c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId==dto.userId}">
    ...
    var url = "<%=cp%>/webBoard/delete.do?" + params;
...
<c:if test="${sessionScope.member.userId==dto.userId}">
    ...
    var page = "${page}";
    var params = "num="+num+"&page="+page;
    var url = "<%=cp%>/webBoard/update.do?" + params;
...
function listPage(page) {
	var url="<%=cp%>/webBoard/listReply.do";
	var num="${dto.num}";
...
function sendReply() {
	...
	$.ajax({
		type:"POST"
		,url:"<%=cp%>/webBoard/insertReply.do"
		,data:params
		,dataType:"json"
...
function deleteReply(replyNum, pageNo, userId) {
	...
	if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
		var url="<%=cp%>/webBoard/deleteReply.do";
...
<div class="container" role="main">
    <div class="bodyFrame col-sm-11"  style="float:none; margin-left: auto; margin-right: auto;">
	    ...
	    <div class="table-responsive" style="clear: both;">
	                     ...
	                     <tr>
	                         <td colspan="2">
							      ...
							      <c:if test="${not empty preReadDto }">
								      <a href="<%=cp%>/webBoard/article.do?${params}&num=${preReadDto.num}">${preReadDto.subject}</a>
							       </c:if>
	                         </td>
	                     </tr>
	                     <tr>
	                         <td colspan="2" style="border-bottom: #d5d5d5 solid 1px;">
							      ...
							      <c:if test="${not empty nextReadDto }">
								      <a href="<%=cp%>/webBoard/article.do?${params}&num=${nextReadDto.num}">${nextReadDto.subject}</a>
							       </c:if>
	                         </td>
	                     </tr>
	                </tbody>
	                	...
	                	<tr>
	                		...
	                		<td align="right">
	                		    <button type="button" class="btn btn-default btn-sm wbtn"
	                		                onclick="javascript:location.href='<%=cp%>/webBoard/list.do?${params}';"> 목록으로 </button>
	                		</td>
	                	</tr>
	                </tfoot>
	            </table>
	       ...
	       <div class="webboard-reply">
	           <div class="webboard-reply-write">
