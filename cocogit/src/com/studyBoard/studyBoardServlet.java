...
public class studyBoardServlet extends MyServlet {
	...
	private void list(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 리스트 글번호 만들기
		int listNum, n = 0;
		Iterator<studyBoardDTO> it = list.iterator();
		while (it.hasNext()) {
			studyBoardDTO dto = it.next();
		...
		// 페이징 처리
		String listUrl = cp + "/studyBoard/list.do";
		String articleUrl = cp + "/studyBoard/article.do?page=" + current_page;
		if (params.length() != 0) {
			listUrl += "?" + params;
			articleUrl += "&" + params;
		}
		...
		// JSP로 포워딩
		String path = "/WEB-INF/views/studyBoard/list.jsp";
	...
	private void createdForm(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String path = "/WEB-INF/views/studyBoard/created.jsp";
		forward(req, resp, path);
	}
	...
	private void createdSubmit(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		resp.sendRedirect(cp + "/studyBoard/list.do");
	}
	...
	private void article(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 게시물 가져오기
		studyBoardDTO dto = dao.readBoard(num);
		if (dto == null) { // 게시물이 없으면 다시 리스트로
			resp.sendRedirect(cp + "/studyBoard/list.do?page=" + page);
			return;
		}
		...
		// 이전글 다음글
		studyBoardDTO preReadDto = dao.preReadBoard(dto.getNum(), searchKey, searchValue);
		studyBoardDTO nextReadDto = dao.nextReadBoard(dto.getNum(), searchKey, searchValue);
		...
		// 포워딩
		String path = "/WEB-INF/views/studyBoard/article.jsp";
		forward(req, resp, path);
	}
	...
	private void delete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// studyBoardReply 테이블은 ON DELETE CASCADE 옵션으로 studyBoard 테이블의 데이터가 지워지면 자동 지워짐
		dao.deleteBoard(num);
		resp.sendRedirect(cp + "/studyBoard/list.do?page=" + page);
	}
	...
	private void listReply(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 포워딩
		String path = "/WEB-INF/views/studyBoard/listReply.jsp";
		forward(req, resp, path);
	}
