...
public class webBoardServlet extends MyServlet {
	...
	private void list(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 페이징 처리
		String listUrl = cp + "/webBoard/list.do";
		String articleUrl = cp + "/webBoard/article.do?page=" + current_page;
		if (params.length() != 0) {
			listUrl += "?" + params;
			articleUrl += "&" + params;
		}
		...
		// JSP로 포워딩
		String path = "/WEB-INF/views/webBoard/list.jsp";
	...
	private void createdForm(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String path = "/WEB-INF/views/webBoard/created.jsp";
		forward(req, resp, path);
	}
	...
	private void article(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 게시물 가져오기
		webBoardDTO dto = dao.readBoard(num);
		if (dto == null) { // 게시물이 없으면 다시 리스트로
			resp.sendRedirect(cp + "/webBoard/list.do?page=" + page);
			return;
		}
		...
		// 이전글 다음글
		webBoardDTO preReadDto = dao.preReadBoard(dto.getNum(), searchKey, searchValue);
		webBoardDTO nextReadDto = dao.nextReadBoard(dto.getNum(), searchKey, searchValue);
		...
		// 포워딩
		String path = "/WEB-INF/views/webBoard/article.jsp";
		forward(req, resp, path);
	}
	...
	private void delete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// webboardReply 테이블은 ON DELETE CASCADE 옵션으로 webboard 테이블의 데이터가 지워지면 자동 지워짐
		dao.deleteBoard(num);
		resp.sendRedirect(cp + "/webBoard/list.do?page=" + page);
	}
	...
	private void listReply(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 포워딩
		String path = "/WEB-INF/views/webBoard/listReply.jsp";
		forward(req, resp, path);
	}
